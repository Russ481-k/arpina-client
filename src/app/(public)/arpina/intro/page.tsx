...
const IntroSection = ({
  ...
  <Box className="intro-box" mb={{ base: 10, md: 15, lg: 20 }}>
    ...
    >
      <Box
        className="image-box"
        w={{ base: "100%", lg: "49.375%" }}
        overflow="hidden"
      >
        <Image src={imageSrc} alt={imageAlt} objectFit="cover" />
      </Box>
      <Box className="text-box" w={{ base: "100%", lg: "49.375%" }} p={4}>
        <Text
          color="#FAB20B"
      ...
      <Box className="intro-box">
        <Box>
          <Flex
            ...
            gap={{ base: 5, md: 8, lg: 10 }}
          >
            <Box className="text-box" w={{ base: "100%", lg: "auto" }}>
              <Text
                color="#FAB20B"
                fontSize={{ base: "sm", md: "md", lg: "md" }}
                fontWeight="700"
            ...
            {/* 첫 번째 행 이미지 */}
            {floorImages.slice(0, 3).map((item, index) => (
              <FloorImageBox
                key={index}
                src={item.src}
                alt={`층별 안내 이미지 ${index + 1}`}
                description={item.description}
                {...(index === 0 ? { marginStart: "auto" } : {})}
              />
            ))}
          </Flex>
          ...
          >
            {floorImages.slice(3).map((item, index) => (
              <FloorImageBox
                key={index + 3}
                src={item.src}
                alt={`층별 안내 이미지 ${index + 4}`}
                description={item.description}
              />
            ))}
          </Flex>
