...
@RestController
@RequestMapping("/bbs/voice/read/{nttId}/comments") // URL을 댓글 리소스로 명확히 분리
@RequiredArgsConstructor
public class VoiceCommentController {
    private final VoiceCommentService commentService;
    private static final Logger logger = LoggerFactory.getLogger(VoiceCommentController.class);
    ...
    public ResponseEntity<VoiceCommentDto> createComment(
        ...
        logger.info("POST /bbs/voice/read/{}/comments is called", nttId);
    ...
    public ResponseEntity<List<VoiceCommentDto>> getComments(@PathVariable Long nttId) {
        logger.info("GET /bbs/voice/read/{}/comments is called", nttId);
    ...
    public ResponseEntity<Void> updateComment(
            ...
            @AuthenticationPrincipal UserDetails userDetails,
            HttpServletRequest httpRequest) {
        logger.info("PUT /bbs/voice/read/{}/comments/{} is called", nttId, commentId);
        String adminId = userDetails.getUsername();
        String ipAddress = httpRequest.getRemoteAddr();
    ...
    public ResponseEntity<Void> deleteComment(
            @PathVariable Long nttId,
            @PathVariable Long commentId) {
        logger.info("DELETE /bbs/voice/read/{}/comments/{} is called", nttId, commentId);
